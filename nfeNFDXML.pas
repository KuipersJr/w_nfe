...
type
  ...
  // This event is used in the TnfeNFDXML.OnNodeCompare event, and should
  ...
  // The TXmlNode represents an element in the XML file. Each TnfeNFDXML holds
  ...
  private
    ...
    FDocument: TnfeNFDXML;         // Pointer to parent XmlDocument
  ...
  protected
    ...
    procedure ChangeDocument(ADocument: TnfeNFDXML);
  public
    // Create a new TXmlNode object. ADocument must be the TnfeNFDXML that is
    // going to hold this new node.
    constructor Create(ADocument: TnfeNFDXML); virtual;
    // \Create a new TXmlNode with name AName. ADocument must be the TnfeNFDXML
    // that is going to hold this new node.
    constructor CreateName(ADocument: TnfeNFDXML; const AName: AnsiString); virtual;
    // \Create a new TXmlNode with name AName and string value AValue. ADocument
    // must be the TnfeNFDXML that is going to hold this new node.
    constructor CreateNameValue(ADocument: TnfeNFDXML; const AName, AValue: AnsiString); virtual;
    // \Create a new TXmlNode with XML element type AType. ADocument must be the
    // TnfeNFDXML that is going to hold this new node.
    constructor CreateType(ADocument: TnfeNFDXML; AType: TXmlElementType); virtual;
    ...
    // node. You can also Assign a TnfeNFDXML document to the node, in that case
    // the RootNodeList property of the TnfeNFDXML object will be copied.
    ...
    // recursively freed when TnfeNFDXML.Free is called.
    ...
    // (name, value, attributes). If the TnfeNFDXML property UtfEncoded is True,
    ...
    // attributes). If the TnfeNFDXML property UtfEncoded is True, all
    ...
    // to a normal ANSI string. If the TnfeNFDXML property UtfEncoded is True,
    ...
    // to a widestring. If the TnfeNFDXML property UtfEncoded is True, all
    // character codes higher than $FF are preserved.
    ...
    property Document: TnfeNFDXML read FDocument write FDocument;
  ...
  // TnfeNFDXML is the XML document holder. Create a TnfeNFDXML and then use
  ...
  protected
    procedure CopyFrom(Source: TnfeNFDXML); virtual;
  ...
  public
    ...
    // Destroy will free all data in the TnfeNFDXML object. This includes the
    // root node and all subnodes under it. Do not call Destroy directly, call
    // Free instead.
    destructor Destroy; override;
    // When calling Assign with a Source object that is a TnfeNFDXML, will cause
    ...
    // begin
    //   if (Node.Name = 'LastNode') and (Sender is TnfeNFDXML) then
    //     TnfeNFDXML(Sender).AbortParsing := True;
    // end;
    // </code>
...
begin
  ...
  begin
    ...
    if Source is TnfeNFDXML then
    begin
      Assign(TnfeNFDXML(Source).FRootNodes);
    end else
      inherited;
end;
...
procedure TXmlNode.ChangeDocument(ADocument: TnfeNFDXML);
var
  i: integer;
begin
  FDocument := ADocument;
...
constructor TXmlNode.Create(ADocument: TnfeNFDXML);
begin
  inherited Create;
  FDocument := ADocument;
end;

constructor TXmlNode.CreateName(ADocument: TnfeNFDXML;
...
constructor TXmlNode.CreateNameValue(ADocument: TnfeNFDXML; const AName,
  AValue: AnsiString);
begin
  Create(ADocument);
  Name := AName;
...
constructor TXmlNode.CreateType(ADocument: TnfeNFDXML;
...
procedure TnfeNFDXML.Assign(Source: TPersistent);
  // local
  procedure SetDocumentRecursively(ANode: TXmlNode; ADocument: TnfeNFDXML);
  var
    i: integer;
  begin
...
begin
  ...
  begin
    ...
    FBinaryEncoding := TnfeNFDXML(Source).FBinaryEncoding;
    FDropCommentsOnParse := TnfeNFDXML(Source).FDropCommentsOnParse;
    FExternalEncoding := TnfeNFDXML(Source).FExternalEncoding;
    FParserWarnings := TnfeNFDXML(Source).FParserWarnings;
    FIndentString := TnfeNFDXML(Source).FIndentString;
    FUseFullNodes := TnfeNFDXML(Source).FUseFullNodes;
    FUtf8Encoded := TnfeNFDXML(Source).FUtf8Encoded;
    FWriteOnDefault := TnfeNFDXML(Source).FWriteOnDefault;
    FXmlFormat := TnfeNFDXML(Source).FXmlFormat;
    FSortAttributes := TnfeNFDXML(Source).FSortAttributes;
    // Assign root
    FRootNodes.Assign(TnfeNFDXML(Source).FRootNodes);
...
procedure TnfeNFDXML.CopyFrom(Source: TnfeNFDXML);
begin
...
constructor TnfeNFDXML.CreateName(const ARootName: AnsiString);
begin
  Create;
  Root.Name := ARootName;
end;

destructor TnfeNFDXML.Destroy;
begin
  FreeAndNil(FRootNodes);
  inherited;
end;

procedure TnfeNFDXML.DoNodeLoaded(Node: TXmlNode);
begin
  if assigned(FOnNodeLoaded) then
    FOnNodeLoaded(Self, Node);
end;

procedure TnfeNFDXML.DoNodeNew(Node: TXmlNode);
begin
  if assigned(FOnNodeNew) then
    FOnNodeNew(Self, Node);
end;

procedure TnfeNFDXML.DoProgress(Size: integer);
begin
  if assigned(FOnProgress) then
    FOnProgress(Self, Size);
end;

procedure TnfeNFDXML.DoUnicodeLoss(Sender: TObject);
begin
  if assigned(FOnUnicodeLoss) then
    FOnUnicodeLoss(Self);
end;

function TnfeNFDXML.GetCommentString: AnsiString;
...
function TnfeNFDXML.GetEncodingString: AnsiString;
begin
  Result := '';
...
function TnfeNFDXML.GetEntityByName(AName: AnsiString): AnsiString;
var
  i, j: integer;
begin
  Result := '';
...
function TnfeNFDXML.GetRoot: TXmlNode;
begin
  Result := FRootNodes.NodeByElementType(xeNormal);
end;

function TnfeNFDXML.GetStyleSheetNode: TXmlNode;
begin
...
function TnfeNFDXML.GetVersionString: AnsiString;
begin
  Result := '';
  if FRootNodes.NodeCount > 0 then
    if FRootNodes[0].ElementType = xeDeclaration then
...
function TnfeNFDXML.IsEmpty: boolean;
var
  R: TXmlNode;
begin
  Result := True;
...
function TnfeNFDXML.LineFeed: AnsiString;
begin
  case XmlFormat of
  xfReadable:
    Result := #13#10;
...
procedure TnfeNFDXML.LoadFromFile(const FileName: AnsiString);
var
  S: TStream;
begin
...
procedure TnfeNFDXML.LoadFromStream(Stream: TStream);
var
...
procedure TnfeNFDXML.ParseDTD(ANode: TXmlNode; S: TStream);
...
procedure TnfeNFDXML.ReadFromStream(S: TStream);
var
  i: integer;
  Node: TXmlNode;
  Enc: AnsiString;
...
procedure TnfeNFDXML.ReadFromString(const AValue: AnsiString);
var
  S: TStream;
begin
  S := TsdStringStream.Create(AValue);
...
procedure TnfeNFDXML.ResolveEntityReferences;
begin
  if assigned(Root) then
    Root.ResolveEntityReferences;
end;

procedure TnfeNFDXML.SaveToFile(const FileName: AnsiString);
var
  S: TStream;
begin
...
procedure TnfeNFDXML.SaveToStream(Stream: TStream);
var
...
procedure TnfeNFDXML.SetCommentString(const Value: AnsiString);
...
procedure TnfeNFDXML.SetDefaults;
begin
  // Defaults
...
procedure TnfeNFDXML.SetEncodingString(const Value: AnsiString);
var
  Node: TXmlNode;
begin
...
procedure TnfeNFDXML.SetVersionString(const Value: AnsiString);
var
  Node: TXmlNode;
begin
...
procedure TnfeNFDXML.WriteToStream(S: TStream);
var
  i: integer;
begin
...
function TnfeNFDXML.WriteToString: AnsiString;
var
  S: TsdStringStream;
begin
  S := TsdStringStream.Create('');
